...
| `RECOVERY_FAILED` on redb, `large` shape, on ext4 `data=ordered`, xfs and btrfs | Phase 4 sweep | **Undecided** | redb aborts on an image whose file is shorter than its header's layout. Whether the image is legal is unresolved. See above. |
...
and it took 10,942 states across four filesystems and five workload shapes to reach it. It
appears in 3 of the 20 combinations and in 11 of the 704 states of each.
