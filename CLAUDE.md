...
- [x] Every violation reproduces on a clean machine from the artifact alone. Verified on a
      VM freshly provisioned from `vm/lima.yaml` (the environment this project's Requirements
      section requires — Linux, ext4/xfs/btrfs, loop devices, a Rust toolchain,
      `/var/lib/crashfuzz` writable) that had never run any part of this pipeline: the
      packaged artifact directory and the repo were copied over, and one command —
      `CRASHFUZZ_REPO=... bash run.sh`, nothing else — built `redb-query` from its own crate
      and reproduced the recorded panic exactly, in 6 seconds. "Clean" means never having run
      this pipeline, not zero prerequisites — a bare machine without the toolchain or a
      writable `/var/lib/crashfuzz` fails loudly on `set -euo pipefail` rather than silently,
      which is the honest floor for a tool whose entire pipeline already depends on that
      layout.
...
- [x] At least 10,000 crash states tested on the primary target — 10,942 across 20
      filesystem/shape combinations, in 21 minutes
...
- [x] Every candidate violation triaged into: real bug, our model wrong, or undecided.
      Undecided is an allowed and honest outcome, but it is not where this one landed. See
      `docs/findings.md`: seven defects of our own (six model defects plus one materialization
      imprecision Phase 5 verification caught), one deliberate bug in our own positive
      control, and one real bug in released redb.

### Phase 5: Disclosure

Only for violations that survive Phase 4 triage. One did: `RECOVERY_FAILED` on redb,
resolved from undecided to a real, independently-rediscovered bug — see
`docs/disclosure.md`.
...
- [x] Reproducer runs from a fresh clone with one command — one command on the environment
      this project's Requirements section documents (Linux, a Rust toolchain,
      `/var/lib/crashfuzz` writable); a review pass caught `run.sh` silently depending on a
      pre-existing build under that same requirement, fixed so it now builds the query tool
      itself when absent. Verified on the same clean VM as the Phase 3 box above.
- [x] Report states: the guarantee (quoted from their docs), the workload, the crash point,
      the expected state, the observed state, and the environment — `docs/disclosure.md`
- [x] The report does *not* speculate about their internals or propose a fix unless asked
- [ ] Filed, with the thread linked in the README regardless of outcome. **Deliberately
...
- [x] `README.md`: what it does, how to run it, results table, honest limitations
- [x] `docs/findings.md`: bugs found or, if none, exactly what was verified clean and what
      that verification is worth
- [x] Architecture diagram of trace to graph to state to oracle — mermaid in `README.md`,
      rendered locally with `@mermaid-js/mermaid-cli` to confirm it parses (a code review
      caught the first draft using `graph` as a node id, a reserved mermaid keyword, which
      would have failed to render on GitHub)
- [x] The failure log: at least three things that did not work, kept in — five entries,
      `docs/findings.md`
- [x] A stranger can reproduce the headline result from the README alone — `bun run
      campaign` (Phase 4, established) produces the full 10,942-state result end to end;
      replaying the specific packaged finding via `run.sh` was re-verified this session on a
      VM with no prior build, one command, 6 seconds
...
| What bugs were found, and their disclosure status | `docs/findings.md`, `docs/disclosure.md` |
...
Commits, newest first (`git log --oneline` is authoritative; update this block when it
...
7c47a1b docs: the exact mechanism behind the undecided redb candidate
d976b3d phase4: reproducers query the target the finding came from
3618644 docs: triage the redb candidate as undecided, record the fdatasync divergence
a8b750c phase4: cf enumerate reports what the bounds select for a trace
44a36b9 docs: phase 4 journal entry and status
8df3776 phase4: delete images for states that found nothing, coverage and findings docs
12e5422 phase4: sweep driver, mount options, and two more oracle false positives
f1d44d2 phase4: redb workload and query tool for the primary target
a2d07d6 docs: phase 3 journal entry, gate status, coverage gaps
addbf3b phase3: end-to-end positive control detects the unsafe rename protocol
f910595 phase3: self-contained reproducer artifacts
03407b4 phase3: end-to-end campaign, control target comes back clean
8e653d2 phase3: sqlite control workload, marker channel, recovery query
0f623ea phase3: acknowledgement log, oracle, root-cause dedup
...
| `core/tools/ftruncate-vs-overwrite.ts` | Phase 5: demonstrates the redb finding's reordering on a real ext4, with `data=journal` as the negative control. |
...
`coverage.md`, `journal.md` (eight entries), `findings.md`, `disclosure.md`.

77 tests across 26 files, all passing. The two control tests are the slow ones: each
...
10,942 crash states on redb across ext4 `data=ordered`, ext4 `data=journal`, xfs and btrfs,
and five workload shapes, in 21 minutes. The table is `docs/results.md`, regenerated by
`bun run campaign` rather than edited.

**Three findings, one signature: a real bug.** `RECOVERY_FAILED|missing:ftruncate@main.redb`
fires on the `large` shape on ext4 `data=ordered`, xfs and btrfs, and on no other shape or
filesystem. redb aborts on an image whose file is shorter than its own header's layout,
permanently losing a database that was fully recoverable. It reproduces from the packaged
image with one command, on a machine that has never run this pipeline. See `docs/findings.md`.

### What Phase 5 established

The candidate above resolved from undecided to a real bug. Three pieces of evidence, all
2026-08-16, all in the Phase 5 journal entry: the reordering was demonstrated directly on a
stock ext4 `data=ordered` filesystem (`core/tools/ftruncate-vs-overwrite.ts`, with
...
`ftruncate` — could not have actually produced. Corrected and recorded in `docs/model.md`;
the finding against released redb is unaffected since the panic fires regardless of the
exact length.
...
our own positive control, and one is the real redb bug.
...
- The redb finding is a real bug, not undecided. It was resolved 2026-08-16 by a block-level
  ext4 experiment, upstream archaeology, and a cross-version test against fixed redb master —
  not by an argument alone. Do not reopen the "is this legal" question without new evidence;
  see the Phase 5 journal entry.
- The materializer over-approximates file length: a kept write past persisted EOF extends
  the file, but ext4's journal would have lost that size effect along with any dropped
  earlier metadata change. This is Reading A in the Phase 5 journal entry's model decision;
