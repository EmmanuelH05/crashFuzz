...
| 4 — The campaign | Complete, all four exit criteria met. 10,942 states, 3 findings, all one signature and all undecided |
| 5 — Disclosure | Does not apply: no violation survived triage as a confirmed bug, so there is nothing to file |
...
**Three findings, one signature, all undecided.** `RECOVERY_FAILED|missing:ftruncate@main.redb`
fires on the `large` shape on ext4 `data=ordered`, xfs and btrfs, and on no other shape or
filesystem. redb aborts on an image whose file is shorter than its own header's layout. It
reproduces from the packaged image with one command. Whether that image is legal is
unresolved, so nothing is filed. See `docs/findings.md`.

Seven candidates have been triaged across Phases 3 and 4: six were defects in this tool, one
is the deliberate bug in our own positive control, and one is the undecided redb candidate.
No confirmed bug in a real target, so Phase 5 does not apply.
