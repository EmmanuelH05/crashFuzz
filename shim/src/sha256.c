/*
 * SHA-256 (FIPS 180-4), single-shot. No allocation, so it is safe to call from
 * the interposed call path.
 */
...
void sha256_hex(const void *data, size_t len, char out[SHA256_HEX_BYTES])
{
    ...
    for (size_t i = 0; i < full_blocks; i++)
        compress(state, bytes + i * 64);

    /* Final block: remaining bytes, 0x80, zero padding, 64-bit bit count. */
    uint8_t tail[128];
    size_t rest = len - full_blocks * 64;
    memcpy(tail, bytes + full_blocks * 64, rest);
    tail[rest] = 0x80;
    ...
    for (int i = 0; i < 8; i++) {
        for (int b = 0; b < 4; b++) {
            uint8_t byte = (uint8_t)(state[i] >> (24 - 8 * b));
